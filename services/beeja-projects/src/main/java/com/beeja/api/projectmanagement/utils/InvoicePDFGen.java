...
public class InvoicePDFGen {
    ...
    public byte[] generatePDF(Contract contract, Invoice invoice,Client client) {
        ...
        Map<String, Object> map = (Map<String, Object>) UserContext.getLoggedInUserOrganization().get("address");
        String billingFromAddress = map.get("addressOne") + ", " + map.get("city") + ", " + map.get("state") + ", " + map.get("pinCode") + ", " + map.get("country");
        ...
        String billingToAddress = clientAddress != null
                ? clientAddress.getStreet() + ", " + clientAddress.getCity() + ", " + clientAddress.getState() + ", " + clientAddress.getPostalCode() + ", " + clientAddress.getCountry()
                : " ";
        ...
        Table idSection = new Table(UnitValue.createPercentArray(new float[]{1, 1, 1}))
        ...
        idSection.addCell(new Cell().add(new Paragraph()
                        .add(new Text("Remittance Ref ").setFontSize(10).setBold().setFontColor(grayColor))
        ...
        Table footerTable = new Table(UnitValue.createPercentArray(new float[]{70, 30})).useAllAvailableWidth();
        ...
        Paragraph invoiceDesc = new Paragraph()
        ...
        Table itemsTable = new Table(UnitValue.createPercentArray(new float[]{10, 30, 40, 20}))
                .useAllAvailableWidth();

        itemsTable.addHeaderCell(
                new Cell().add(new Paragraph("S.No.").setFontSize(10).setBold())
                        .setFontColor(grayColor)
                        .setBorder(Border.NO_BORDER)
        );
        itemsTable.addHeaderCell(
                new Cell().add(new Paragraph("Task").setFontSize(10).setBold())
                        .setFontColor(grayColor)
                        .setBorder(Border.NO_BORDER)
        );
        itemsTable.addHeaderCell(
                new Cell().add(new Paragraph("Description").setFontSize(10).setBold())
                        .setFontColor(grayColor)
                        .setBorder(Border.NO_BORDER)
        );
        itemsTable.addHeaderCell(
                new Cell().add(new Paragraph("Price (€)").setFontSize(10).setBold())
                        .setTextAlignment(TextAlignment.RIGHT)
                        .setFontColor(grayColor)
                        .setBorder(Border.NO_BORDER)
        );
        ...
        if (tasks != null) {
            for (Task task : tasks) {
                itemsTable.addCell(
                        new Cell().add(new Paragraph(String.valueOf(serialNo++)).setFontSize(9))
                                .setTextAlignment(TextAlignment.CENTER)
                                .setBorder(Border.NO_BORDER)
                );
                itemsTable.addCell(
                        new Cell().add(new Paragraph(task.getTaskName()).setFontSize(9))
                                .setBorder(Border.NO_BORDER)
                );
                itemsTable.addCell(
                        new Cell().add(new Paragraph(task.getDescription()).setFontSize(9))
                                .setBorder(Border.NO_BORDER)
                );
                itemsTable.addCell(
                        new Cell().add(new Paragraph(String.valueOf(task.getPrice())).setFontSize(9))
                                .setTextAlignment(TextAlignment.RIGHT)
                                .setBorder(Border.NO_BORDER)
                );
        ...
        Table calculationTable = new Table(UnitValue.createPercentArray(new float[]{50, 50}))
                .setWidth(UnitValue.createPercentValue(25))
                .setHorizontalAlignment(HorizontalAlignment.RIGHT)
                .setMarginTop(10);

        calculationTable.addCell(new Cell()
        ...
        Paragraph amountInWordsRupeesPara = new Paragraph()
        ...
        Paragraph notePara = new Paragraph()
                ...
                .add(new Text("Please transfer the due amount to the following bank account within next ").setFontSize(9).setFontColor(grayColor))
        ...
        paymentTable.addCell(new Cell().add(new Paragraph("Name")).setBorder(Border.NO_BORDER).setFontColor(grayColor));
        paymentTable.addCell(new Cell().add(new Paragraph(orgName)).setBorder(Border.NO_BORDER).setBold().setFontColor(blackColor));

        paymentTable.addCell(new Cell().add(new Paragraph("Bank Name")).setBorder(Border.NO_BORDER).setFontColor(grayColor));
        paymentTable.addCell(new Cell().add(new Paragraph(bankName)).setBorder(Border.NO_BORDER).setBold().setFontColor(blackColor));

        paymentTable.addCell(new Cell().add(new Paragraph("Account Number")).setBorder(Border.NO_BORDER).setFontColor(grayColor));
        paymentTable.addCell(new Cell().add(new Paragraph(accountNo)).setBorder(Border.NO_BORDER).setBold().setFontColor(blackColor));

        paymentTable.addCell(new Cell().add(new Paragraph("IFSC")).setBorder(Border.NO_BORDER).setFontColor(grayColor));
        paymentTable.addCell(new Cell().add(new Paragraph(IFSC)).setBorder(Border.NO_BORDER).setBold().setFontColor(blackColor));
        ...
        Paragraph remarksPara = new Paragraph()
                ...
                .add(new Text("\"Thank you so much for the great opportunity as always.\"")
                        .setFontSize(9)
                        .setFontColor(grayColor))
                .setTextAlignment(TextAlignment.LEFT)
                .setMarginBottom(15);
        ...
        Paragraph wishingPara = new Paragraph()
