...
class FileServiceImplTest {
  ...
  void listOfFileByEntityId_Success_WithReadAllDocumentsPermission() {
    UserContext.setLoggedInUser(
        ...
        "emp1",
        new HashMap<>(),
        new HashSet<>(Collections.singletonList(PermissionConstants.READ_ALL_DOCUMENTS)),
        "token");
  ...
  void listOfFileByEntityId_FeignClientException() {
    ...
    FeignClientException thrown =
        assertThrows(
            FeignClientException.class, () -> fileService.listOfFileByEntityId("entity456", 1, 10));
  ...
  void listOfFileByEntityId_UnAuthorisedException() {
    UserContext.setLoggedInUser(
        ...
        "emp1",
        new HashMap<>(),
        new HashSet<>(Collections.singletonList("SOME_OTHER_PERMISSION")),
        "token");
  ...
  void uploadFile_Success() throws Exception {
    ...
    when(fileClient.uploadFile(any(FileUploadRequest.class)))
        .thenReturn(ResponseEntity.ok(mockFileMap));
  ...
  void uploadFile_FeignClientException() {
    when(fileClient.uploadFile(any(FileUploadRequest.class)))
        .thenThrow(new RuntimeException("Upload service down"));

    FeignClientException thrown =
        assertThrows(
            FeignClientException.class, () -> fileService.uploadFile(mockFileUploadRequest));
  ...
  void deleteFile_Success_DeleteAllDocumentPermission() throws Exception {
    UserContext.setLoggedInUser(
        ...
        "differentEmployee",
        new HashMap<>(),
        new HashSet<>(Collections.singletonList(PermissionConstants.DELETE_ALL_DOCUMENT)),
        "token");
  ...
  void deleteFile_Success_OwnDocument() throws Exception {
    UserContext.setLoggedInUser(
        ...
        "employee123",
        new HashMap<>(),
        new HashSet<>(Collections.singletonList(PermissionConstants.DELETE_DOCUMENT)),
        "token");
  ...
  void deleteFile_ObjectMapperThrowsException() throws Exception {
    ...
    Exception thrown = assertThrows(Exception.class, () -> fileService.deleteFile("file123"));
