...
/**
 * Configuration class responsible for performing startup checks and data fixes on the Inventory collection.
 * <p>
 * Specifically, this class ensures that every Inventory document has a non-null {@code created_at} field.
 * If any document is missing the {@code created_at} timestamp, it will be automatically updated with the current system date.
 * <p>
 * This check runs only once during application startup.
 * <p>
 * Note: This uses MongoTemplate to avoid deserializing Inventory objects, thereby bypassing issues with default field initialization.
 */
@Configuration
@Slf4j
    ...
    public void assignCreatedDateIfMissing() {
        try {
            ...
            if (!inventoriesWithoutCreatedDate.isEmpty()) {
                log.info("Updated created_at for {} inventory items.", inventoriesWithoutCreatedDate.size());
            }
        } catch (Exception e) {
